    ...
    private static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal userMeal : mealList) {
            ...
            for (UserMeal listOfTheDay : mealList) {
                if (userMeal.getDateTime().toLocalDate().equals(listOfTheDay.getDateTime().toLocalDate())) {
                    calories = calories + listOfTheDay.getCalories();
                }
                if (TimeUtil.isBetween(userMeal.getDateTime().toLocalTime(), startTime, endTime)) {
                    listOfTheTime.add(new UserMealWithExceed(userMeal.getDateTime(), userMeal.getDescription(),
